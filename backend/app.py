...
def get_cors_origins(raw_origins):
    if not raw_origins:
        return "*"
...
def create_app():
    ...
    cors_origins = get_cors_origins(app.config["FRONTEND_URL"])
...
def register_routes(app):
    ...
    def list_jobs():
        query = request.args.get("query", "").strip().lower()
        jobs = JobPosting.query.order_by(JobPosting.is_featured.desc(), JobPosting.created_at.desc()).all()
        if query:
            jobs = [
                job for job in jobs if query in " ".join(
                    [job.title, job.team, job.location, job.type, job.description]
                ).lower()
            ]
        return jsonify([job_to_dict(item) for item in jobs])
...
def seed_data():
    ...
    if JobPosting.query.count() == 0:
        db.session.add_all(
            [
                JobPosting(
                    title="Global Customer Success Associate",
                    team="Customer Success",
                    location="Remote / Nairobi",
                    type="Full-time",
                    description="Support employers and workers across onboarding, job applications, and premium hiring flows.",
                    apply_url="https://example.com/jobs/customer-success",
                    is_featured=True,
                ),
                JobPosting(
                    ...
                    location="Nairobi",
                    type="Full-time",
                    description="Coordinate premium marketplace operations, provider quality, and fast-turnaround service workflows.",
                    apply_url="https://example.com/jobs/field-operations",
                    is_featured=True,
                ),
                JobPosting(
                    title="Frontend Marketplace Engineer",
                    team="Engineering",
                    location="Remote",
                    type="Contract",
                    description="Build polished user experiences for the service marketplace, employer portal, and multilingual tooling.",
